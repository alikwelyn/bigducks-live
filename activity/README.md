...
- Consumo é **informativo e nunca bloqueante**: o Durable Object soma os bytes de relay que encaminha e, a cada 30 s, recebe do espectador uma estimativa dos bytes que o **SFU** lhe entregou (a perna P2P não entra, porque não passa pela Cloudflare). O total é guardado por mês e mostrado apenas ao transmissor, na forma "Consumo do mês (estimado)". **Nada limita, reduz ou encerra uma transmissão por causa disso.** Limitações honestas: a parcela de SFU é auto-relatada pelo navegador, o primeiro intervalo de cada sessão não é contado e relatos mais frequentes que 25 s são ignorados, então o número tende a **subestimar**; ele é por sala, não da conta; e a gravação em disco é no máximo uma vez por minuto, então um reinício pode perder o último trecho. Na direção oposta, um espectador mal-intencionado consegue inflar a estimativa: ela serve para acompanhar a ordem de grandeza, não para auditoria. Confirme sempre na fatura da Cloudflare.
- O status do estúdio passa por um `status-view` que garante `role="status"` e `aria-live`, com estado visível (`data-state`) para erro — antes uma falha era escrita com a mesma cor de sucesso e nada era anunciado. As opções de qualidade vêm de `quality-options.js`, cujo teste exige que cada valor exista em `profileFor`; o teste antigo do `main.js` era tautológico e foi removido.
- Versão e códigos: a UI mostra a versão **do servidor que está no ar** (lida de `activity/package.json` e exposta em `/api/config`), ao lado da contagem de espectadores e no cabeçalho do estúdio. Cada falha leva um **código curto e estável** (`0x0`–`0xF`, ver `diagnostic-code.js`) para alguém poder reportar "deu 0x5" sem ambiguidade; **um código nunca muda de significado**. O workflow `activity-version.yml` sobe o patch a cada push em `public-main`, com `[skip ci]` no commit gerado para não se auto-disparar.
- Diagnóstico de acesso: `/api/config` informa `guildRestricted`, e uma sessão recusada devolve `code` — `guild_required` (403, verificado e não é membro), `guild_unverifiable` (503, não foi possível verificar: escopo `guilds` ausente, lista ilegível ou `DISCORD_GUILD_ID` fora do formato), `discord_required`/`discord_failed` (401). O cliente mostra uma frase acionável por código e, no caso `guild_unverifiable`, pede consentimento ao Discord **uma vez** e repete a sessão. Julgamentos inconclusivos **não** entram no cache, senão o retry bateria no cache e falharia na hora.
