...
- Consumo é **informativo e nunca bloqueante**: o Durable Object soma os bytes de relay que encaminha e, a cada 30 s, recebe do espectador uma estimativa dos bytes que o SFU entregou. O total é guardado por mês e mostrado apenas ao transmissor, na forma "Consumo do mês (estimado)". **Nada limita, reduz ou encerra uma transmissão por causa disso.** Limitações honestas: a parcela de SFU é auto-relatada pelo navegador e pode subestimar; o número é por sala, não da conta; e o valor só é gravado em disco a cada minuto, então um reinício pode perder o último trecho. Confirme sempre na fatura da Cloudflare.
